...
impl Uncreative {
    ...
    pub fn encrypt_data(data: &[u8], len: usize, key: u16) -> Option<Vec<u8>> {
        ...
        let mut encrypted_content: Vec<u8> = Vec::with_capacity(len + 10);
        ...
        for char in data {
            ...
            let mut encrypted_char: u8 = char.rotate_right(4);
    ...
    pub fn decrypt_data(data: &[u8], len: usize) -> Option<Vec<u8>> {
        ...
        for (i, dat) in data.iter().enumerate().take(len).skip(10) {
            let mut char = *dat;
            char ^= (key << (i % 2 * 8)) as u8;

            char = char.rotate_right(4);
